...
@patch("spark_apps.bronze.decoders.avro_decoder.requests.get")
def test_fetch_schema_requests_latest_topic_value_schema(
    mock_get,
):
    response = MagicMock()
    response.status_code = 200
    response.json.return_value = {"schema": '{"type":"record","name":"Order"}'}

    mock_get.return_value = response

    result = _fetch_schema("transactional.orders")

    mock_get.assert_called_once_with(
        (f"{SCHEMA_REGISTRY_URL}/subjects/transactional.orders-value/versions/latest"),
        timeout=10,
    )

    assert result == ('{"type":"record","name":"Order"}')
...
def test_fetch_schema_returns_none_for_missing_subject(
    ...
    result = _fetch_schema("transactional.returns_refunds")
...
def test_fetch_schema_raises_for_http_error(
    ...
    response.raise_for_status.side_effect = requests.HTTPError("Schema Registry error")
    ...
    with pytest.raises(
        ...
        _fetch_schema("transactional.orders")
...
def test_fetch_schema_propagates_connection_error(
    ...
    mock_get.side_effect = requests.ConnectionError("Schema Registry unavailable")
...
def test_fetch_schema_propagates_timeout(
    ...
    mock_get.side_effect = requests.Timeout("Schema Registry timed out")
...
def test_fetch_schema_caches_result_for_same_topic(
    ...
    first_result = _fetch_schema("transactional.orders")

    second_result = _fetch_schema("transactional.orders")
...
def test_fetch_schema_does_not_share_cache_between_topics(
    ...
    first_response.json.return_value = {"schema": '{"name":"Order"}'}

    second_response = MagicMock()
    second_response.status_code = 200
    second_response.json.return_value = {"schema": '{"name":"Product"}'}
    ...
    order_schema = _fetch_schema("transactional.orders")

    product_schema = _fetch_schema("transactional.products")
...
@patch("spark_apps.bronze.decoders.avro_decoder._fetch_schema")
def test_decode_returns_none_when_schema_is_not_registered(
    ...
    mock_fetch_schema.assert_called_once_with("transactional.returns_refunds")

    df.withColumn.assert_not_called()


@patch("spark_apps.bronze.decoders.avro_decoder.from_avro")
@patch("spark_apps.bronze.decoders.avro_decoder.expr")
@patch("spark_apps.bronze.decoders.avro_decoder._fetch_schema")
def test_decode_removes_confluent_header(
    ...
    mock_fetch_schema.return_value = '{"type":"record","name":"Order","fields":[]}'

    payload_expression = MagicMock(name="payload_expression")
    decoded_expression = MagicMock(name="decoded_expression")
    ...
    mock_expr.assert_called_once_with("substring(raw_value, 6, length(raw_value) - 5)")
...
@patch("spark_apps.bronze.decoders.avro_decoder.col")
@patch("spark_apps.bronze.decoders.avro_decoder.from_avro")
@patch("spark_apps.bronze.decoders.avro_decoder.expr")
@patch("spark_apps.bronze.decoders.avro_decoder._fetch_schema")
def test_decode_flattens_decoded_fields(
    ...
    mock_fetch_schema.return_value = '{"type":"record","name":"Order"}'
    ...
    def build_mock_column(column_name):
        column = MagicMock(name=f"column_{column_name}")
    ...
    mock_col.assert_any_call("_decoded.order_id")
    mock_col.assert_any_call("_decoded.status")
    mock_col.assert_any_call("_decoded.total_amount")

    mocked_columns["_decoded.order_id"].alias.assert_called_once_with("order_id")

    mocked_columns["_decoded.status"].alias.assert_called_once_with("status")

    mocked_columns["_decoded.total_amount"].alias.assert_called_once_with("total_amount")
...
def test_decode_uses_default_value_payload_column(
    ...
    mock_fetch_schema.return_value = '{"type":"record","name":"Event"}'
    ...
    mock_expr.assert_called_once_with("substring(value, 6, length(value) - 5)")
...
def test_decode_propagates_schema_registry_error(
    ...
    mock_fetch_schema.side_effect = requests.ConnectionError("Schema Registry unavailable")
