...
def main() -> None:
    spark = build_iceberg_spark(
        "profile-product-history-consistency"
    )
    ...
    try:
        ...
        missing_count = (
            products_without_history.count()
        )
        ...
        mismatch_count = (
            price_mismatch.count()
        )

        print(
            f"\nCurrent price mismatches: "
            f"{mismatch_count:,}"
        )
        ...
        print()
        print(
            "[PASS] PRODUCT HISTORY "
            "CONSISTENCY PROFILING COMPLETED"
        )
